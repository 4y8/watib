...
support goto) and exceptions. This version is still in draft, but the mentionned
...
see~\cite[Section~2.1]{phipps2023continuing}. This program starts by defining a
type of integer lists (\texttt{\$pair-nil}) which is a super-type of non-empty
lists (\texttt{\$pair}). A non-empty list has a head (field \texttt{\$car})
which is a 32-bit integer and a tail which is a list. The function
\texttt{\$has-zero} checks wether or not a given list contains 0. It returns a
\textsf{i32} as Wasm has no boolean. The emptiness of the list is checked by
checking that the argument of type \texttt{\$pair} using the instruction
\textsf{ref.test}. The head and tail of the list can then be accessed after it
has been cast to the type \texttt{\$pair} using the instruction
\textsf{ref.cast}. Otherwise, the program wouldn't be well-typed.
...
happens often when developping a runtime.
...
(checking some names don't appear twice, globals initial values are constant,
etc.).
\subsection{WebAssembly's type system}
We present the types used in WebAssembly and some declarative rules for
subtyping and typing of instructions. They are all stated in an implicit context
which we will refer as $C$ when needed. We omit its full definition. The only
relevant field in the following is \textsf{types} which maps type indices to
\emph{defined types} as defined in~\ref{deft}.
...
(structures or arrays, called aggregates) or unboxed integers (using tagged
pointers). Each reference type is either nullable ($\refnullt ht$) or not
($\reft ht$), where $ht$ is a \emph{heap type}. Such a type is either an
\emph{abstract heap type} which we do not detail here or a type indix refering
to one of the three following possibilities. \emph{Function types} are written
$\funt{t_1}^*\to{t_2}^*$ (recall a function can have multiple outputs).
\emph{Array types} are written $\arrt mut\ t$, where $mut$ denotes the
mutability of the array. It is \textsf{var} for a mutable array and
$\textsf{const}$ otherwise. \emph{Structure types} are written $\strt
...
for each of its instrution. For instance, the sequence \texttt{(i32.const~0)
...
defined types have to be specified as part of the type defintion to be used.
...
definitions, we build an internal representation where indices refering to types
of the block have been replaced by special indices of the form $\rect i$ where
$i$ is the position of the corresponding index in the block. The concrete
defintions of the types are then replaced by \emph{defined types} consisting of
...
symbol, these types are contined in the file \texttt{Val/instruction-types.sch}.
...
references to $x$ with references to $y$, until the next assignement to $x$ or
$y$. We were inspired by~\cite[Section~12.5]{muchnick1997advanced} approach to
local copy propagation.

We travrese a sequence of instruction while maintaining a table which indicates
...
We evaluate some expressions whose value is known at compile time. We also also
remove conditional branching when the condition is constant.
...
branches to $l$. The block labelled by $l$ puts its parameter in the local
...
\begin{mathpar}
  ...
  \inferrule{\text{\textsf{unroll}} (deftype)= (symbol\ldots)}
            {C\vdash deftype\leq\text{symbol}}\hfill
