...
\begin{abstract}
  ...
  the development of a Wasm backend for the Bigloo Scheme compiler. However,
  this backend relied on binaryen, an external toolchain with user-friendliness
  and spec-compliance issues.
  ...
  former deals with the mentioned issues of the latter.
\end{abstract}

\section{Introduction}
In 2024, a Wasm backend was added to the Bigloo Scheme compiler~\cite{Bigloo}.
It generated textual Wasm and relied on an external toolchain for the assembly.
We developed \textsf{watib} as a replacement for this toolchain. It is part of
the Bigloo compiler and is also available as a standalone tool with a
command-line
interface\footnote{\url{https://www.normalesup.org/~boussaa/watib/}}.

The rest of this section gives an overview of WebAssembly and
\textsf{watib}. Section~\ref{val} details \textsf{watib}'s type checker.
Section~\ref{opt} describes the different optimisation passes.
Section~\ref{bench} compares \textsf{watib}'s and existing tools' performances.
Section~\ref{sbbv} depicts the current state of an optimisation we are working
on. This presentation omits the assembly phase as, once we have an intermediate
representation, it is a straightforward transcription of the specification.
...
language endowed with a portable and efficient representation. Unlike
traditional assembly, Wasm is statically typed and only supports structured
control flow. Wasm support in existing compilers is increasing~\cite{emscripten,
  kotlin, ocaml}.
...
or wasm-tools~\cite{WasmTools} recieve this format. The textual format
...
high-level or functional languages. Wasm has now a garbage collector,
...
or not a given list contains 0. It returns a \textsf{i32} as Wasm has no boolean
type. The function starts by testing the non-emptiness of its argument by
...
library is optional, which is mandatory for integration with the Bigloo
compiler. We have set up continuous integration against tests bundled with the
...
of our goals. \textsf{Watib}'s design allows us to keep up with the additions
made to the language. To add an instruction that isn't a block, one puts a new
entry in the list of opcodes (\texttt{Asm/opcodes.sch}) and in the list of
validation rules (\texttt{Val/instruction-types.sch}). The case of block
instructions is more involved as their presentations in text format differ from
the ones of plain instructions. It should not be a problem because the
introduction of new kinds of blocks is rare. One also has to modify the
optimisation passes to take into account the new instructions.
...
focusing on bynarien (and its assembler \textsf{wasm-as}). It is the toolchain
most compilers use~\cite{Binaryen} and the one Bigloo used before switching to
\textsf{watib}.
...
pass, and avoids having to correct errors one by one. The Bigloo Wasm backend
produced a file of 1,6 million lines. Running \textsf{wasm-as} to find a single
error would take a noticeable amount of time, which complicated debugging. We
also try to give informative error messages. The benefits of these features
become apparent when using our toolchain to correct code written by hand when,
for instance, developing a runtime.
...
\textsf{wasm-as} we witnessed~\cite{WasmAsExtension}; a copy is in
...
using local variables instead by \textsf{wasm-as}}, as these new features can
create a data flow that cannot be reflected by classic ASTs. For instance, in
...
additional burden. We hope that will enable different optimisations and a better
treatment of code written using WebAssembly's new features.
...
Wasm's main types are \emph{value types}, which will be noted $t, t_1, t_2,
\ldots$. They are the types of values on the stack. They can be \emph{numeric
types} (\textsf{i32}, \textsf{f64}, etc.), \emph{vector types} (\textsf{v128})
or \emph{reference types}. The latter category represents pointers to functions,
...
types as long as it adds the same on both. Figure~\ref{subinstr} presents a
formal version of these rules.
...
and most general types, as defined in~\ref{mgt}.
...
instruction that is a subtype of all its types) or are not exited (like
...
information. As we show it later, some code is unnecessary execution-wise but
...
$y$. The Copy Propagation pass perfoms such replacements. We took inspiration from
Muchnick's~\cite[Section~12.5]{muchnick1997advanced} approach to local copy
propagation for our implementation.

We traverse a sequence of instruction while maintaining a table \textsf{acp}
which indicates if a variable contains the value of another one and we associate
each enclosing label to a list of such tables. We then apply the following
procedure when encountering an instruction $i$:
\begin{description}
  \item[assignment] When $x$ is assigned a variable's $y$ value, update
    \textsf{acp} with $x$ pointing to $y$. Otherwise all the assignments
  ...
  \item[loop] When entering a loop, reset \textsf{acp} to remove all the
    ...
    the optimisation we describe in~\ref{sbbv}}.
  \item[if] When encountering an if statement, treat each branch as a block and
    take the upper bound of the two tables.
...
At the end of each block, remove all of the assignments in \textsf{acp} whose
...
This pass eliminates redundant casts and replace type tests by constants if
their result can be determined. Such casts and tests could be introduced by
copy-propagation as when we may replace a variable with another one of a smaller
type or by the next optimisation. To stay conservative about side effects,
eliminated tests are replaced by a \textsf{drop} and the result of the test,
which is a constant. Pure Drop Elimination will remove some useless computation.
Constant folding will propagate the result of the test.
...
$rt_1$ on top of the stack checks it against the type $rt_2$. If the type test
succeeded, branch to label $l$ with the value on top of the stack cast to $rt_2$
as a parameter, otherwise the execution continues with no modification to the
stack.

This pass replaces \textsf{if} instructions which branch on a $(\reftest
rt\ (\localget x))$ by calls to \textsf{br\_on\_cast}. It adds a local variable
$y$ of type $rt$ and creates a new block labelled $l$ with a parameter of type
$rt$. The else branch follows the \textsf{br\_on\_cast} which branches to $l$.
The block labeled by $l$ puts its parameter in the local variable $y$ and
replaces $x$ by $y$ in the then branch following the same procedure as Copy
Propagation. Cast Elimination then removes useless tests. This whole code is
contained in a block which the else branch goes to at the end. Figure~\ref{br}
...
stack is recorded to put type annotations on the basic blocks.
...
\begin{itemize}
  \item The GHC compiler, which uses Ramsey's algorithm, makes CFGs reducible
    ...
    cost of a size expansion, which can be
    exponential~\cite{carter2003folklore}. GHC implements node
    splitting~\cite[Appendix~A]{ramsey2022beyond} with a greedy algorithm. A
    more elaborate method obtains better results~\cite{janssen1997making}.
  \item Non-reducibility appears when SBBV duplicates the head of a loop and
    that there are arcs between the different versions of the same loop. We can
    then make the graph reducible by creating a new node that serve as a common
    head for all the versions of the loop. Instead of jumping to a head, a node
    would branch to this new head indicating which version of the loop it
    targets. The new head would then branch to one of the previous heads with a
    \textsf{br\_table}. This approach doesn't have the same size problems but
